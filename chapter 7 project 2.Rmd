...
```
his is preferred as it makes it easier to read the text. In Section 7.11 we learn a better way of assuring we can see the points and the labels.
...
Here we want to add a line that represents the average murder rate for the entire country. Once we determine the per million rate to be  r, this line is defined by the formula:  y=rx, with  y  and  x
  our axes: total murders and population in millions, respectively. In the log-scale this line turns into:  
log(y)=log(r)+log(x) . So in our plot it’s a line with slope 1 and intercept  log(r). To compute this value, we use our dplyr skills:
```{r}
r <- murders %>% 
...
p + theme_economist()
```
ou can see how some of the other themes look by simply changing the function. For instance, you might try the theme_fivethirtyeight() theme instead.
...
d Because we want all colors to be blue, we do not need to map colors, just use the color argument in 
geom_label.
#####d
