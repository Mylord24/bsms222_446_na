...
3.4 For-loops
The formula for the sum of the series  n(n+1)/2
...
How can we compute  Sn for various values of n, say  n=1,…,25
 ? Do we write 25 lines of code calling compute_s_n? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of  
n
 . For-loops let us define the range that our variable takes (in our example  
n=1,…,10
 ), then change the value and evaluate expression as you loop.
...
```{r}
m<- 25
s_n <- vector(length = m) #creat an empty vector
...
```{r}
nchar(state.name)
new_names <- ifelse(nchar(state.name)>8, state.abb, state.name)
new_names
...
```{r}
altman_plot <- function(x,y){plot(x-y, x+y)}
altman_plot(x,y)
...
```
3
7. Write a function compute_s_n that for any given  
n computes the sum S_n = 1^2 + 2^2 + 3^2 + \dots n^2
...
12. Confirm that the formula for this sum is  
Sn=n(n+1)(2n+1)/6

```{r}

identical(compute_s_n(n), n*(n+1)*(2*n + 1)/6)
