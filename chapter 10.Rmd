...
```{r}
state <- dat$state %>% reorder(dat$rate)
...
```{r}
dat$state <- dat$state %>% reorder(dat$rate)
dat %>% ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
coord_flip()
```
...
2 The graph does not show standarad errors.
...
```{r}
murders %>%  ggplot(aes(reorder(region, rate, median), rate)) + 
  geom_boxplot() +
...
dat %>% 
  ...
  ggtitle("Cases per 10,000 by state") + 
  xlab("") + ylab("") +
  geom_text(data = data.frame(x = 1938, y = 2), 
            mapping = aes(x, y, label="US average"), 
            color="black") + 
...
```{r}
library(dslabs)
us_contagious_diseases %>% filter(disease == "Smallpox" & state == "California")
```
...
```{r}
dat %>% filter(state == "California" & !is.na(rate) & !weeks_reporting <10) %>%
  ggplot(aes(year, rate)) +
  geom_line() + 
  ylab("Cases per 10,000")  + 
  geom_vline(xintercept=1938, col = "blue")
