...
To solve another equation such as   we can copy and paste the code above and then redefine the variables and recompute the solution:
```{r}
a <- 3
b <- 2
c <- -1
...
1. What is the sum of the first 100 positive integers? The formula for the sum of integers   throughis and then use R to compute the sum of   through using the formula. What is the sum?
...
my_df <- data.frame(state= murders$state, ranking = ranks)
my_df
```
...
my_df<- data.frame(state= ind, ranking = sort(ranks))
my_df
...
2. What is the following sum   Hint: thanks to Euler, we know it should be close to  
...
```{r}
murder_rate2 <- murder_rate/100000
murder_rate2
mean(murder_rate2)
mean(murder_rate)
```
...
```{r}
ind <- match(c("California"), murders$state)
...
```{r}
mean(murder_rate)
a <- murder_rate - mean(murder_rate)
b <- a<0
sum(b)
b
```
...
```{r}
match(c("Alaska", "Michigan", "Indiana"), murder_rate )
murders$abb
murders$state[c(2, 23, 15)]
...
```{r}
a <- c("MA", "ME", "MI", "MO", "MU")%in%murders$abb
a
B<-which(a == !TRUE)
B
...
```{r}
x<- with(murders, murders$population/100000)
hist(x)
```
...
boxplot(population/1000~region, data=murders)
```
