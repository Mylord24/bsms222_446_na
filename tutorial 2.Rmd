...
Gene biotype. 0 or 1 based annotation in GTF, BED formatWhy some features have 1 bp length?What is the meaning of zero-length exons in GENCODE? Also fun to have a review for microexonsTranscript support level (TSL)
...
```{r}
a<-d %>% group_by(gene_id) %>% count()
mean(a$n)

a$n %>% quantile(c(0.25, 0.5, 0.75))

d %>% group_by(gene_id) %>% count() %>% arrange(desc(n)) %>% .[1,]
```
...
```{r}
x<- d %>% group_by(gene_type) %>% count()
x
x %>% filter(gene_type %in% c('protein_coding', 'lncRNA', 'pseudogene')) 
```

3. Final task is to compute the number of transcripts per gene per chromosome.

```{r}
d %>% group_by(chrom) %>% count()
```
...
```{r}
quantile(d$width, c(0, 0.25, 0.5, 0.75, 1))
```
...
```{r}
d %>% group_by(d$gene_type, d$width) %>% count()
```
...
With transcript, how many transcripts are categorized for each TSL?


From the first question, please count the number of transcript for each TSL by gene biotype.
...
Please count the number of hgnc by gene biotypes.Please count the number of hgnc by level. Please note that level in this question is not TSL. 

Please find information in this link: 1 (verified loci), 2 (manually annotated loci), 3 (automatically annotated loci).


3.5. Transcripts in the GENCODE
Which gene has the largest number of transcripts?
Please calculate the quantiles (0%, 25%, 50%, 75%, 100%) of the gene length for protein coding genes and long noncoding genes.
Please count the number of transcripts by chromosomes.


3.6. Autosomal vs. Sex chromosomes.
Please calculate the number of genes per chromosome.

Please compare the number of genes between autosomal and sex chromosome (Mean, Median).

Please divide the genes into groups ‘protein coding’ and ‘long noncoding’, and then compare the number of genes in each chromosomes within groups.
