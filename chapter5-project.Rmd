...
```{r}
file.copy("murders.csv",fullpath)
```
...
files
x <- c(1:9)
fullpath <- file.path(path, files[x] )
file.copy( fullpath, files[x]  , overwrite = T)
read_excel("2010_bigfive_regents.xls")
read.csv("carbon_emissions.csv" )
read.csv("fertility-two-countries-example.csv")
read.table("HRlist2.txt")
read.csv("life-expectancy-and-fertility-two-countries-example.csv")
read.csv("murders.csv" )
read.csv("olive.csv")
read.csv("RD-Mortality-Report_2015-18-180531.pdf")
read.csv("ssa-death-probability.csv" )
...
```{r}
dat<-read.csv("olive.csv", header = F)
```
...
```{r}
url <- 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTHVSozssMwor7QJzAGCM_GWF7veZYmb1eSU3pwzNaj7FDEU83s1aLuxN48aOL_Hgr5IuOe8Nce3F60/pubhtml'
read.csv(url)
download.file(url, "weight.exel")
getwd
