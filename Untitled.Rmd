...
Formal way (but slow): you can download the data as described in 2.1Simple ways: get the file from my dropbox link where I downloaded and save to R object as described in 2.2
...
```{r}
load("/Users/nauiju/Downloads/data.TCGA_LUAD_LUSC.gene_expression.Rdata")
```
...
TP: Primary solid TumorTR: Recurrent solid TumorNT: Solid Tissue Normal
...
Q1. Add labels for the plot.Q2. Change the color for categories.Q3. Save to PDF fileQ4. Select the column with continuous information and plot the distribution by yourself.
...
counts_tumor <- d_luad %>% dplyr::count(tumor_stage)
...
bind_rows(d_luad %>% 
          ...
          d_lusc %>% 
            ...
            mutate(type='lusc') %>%
            select(type, tumor_stage, gender)) %>% 
  dplyr::count(type, tumor_stage, gender) %>% 
