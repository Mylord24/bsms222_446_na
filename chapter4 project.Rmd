...
sd(na_example)
class(rate)
summarise(murders, rate = sum(total)/ sum(population))
...
NHANES %>% summarize(average = mean(a, na.rm = TRUE), standard_deviation = sd(a, na.rm = TRUE))
...
select(NHANES, BPSysAve) 
max(NHANES$BPSysAve, na.rm = TRUE)
min(NHANES$BPSysAve, na.rm = TRUE )
NHANES %>% summarize(average = mean(a, na.rm = TRUE), standard_deviation = sd(a, na.rm = TRUE), maxbp=max(NHANES$BPSysAve, na.rm = TRUE), minbp= min(NHANES$BPSysAve, na.rm = TRUE ))
...
NHANES %>% select(Gender, AgeDecade, BPSysAve)%>% group_by(AgeDecade) %>% filter(Gender == 'female') %>% summarise(avarage = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```

5. Repeat exercise 4 for males.
```{r}
NHANES %>% select(Gender, AgeDecade, BPSysAve)%>% group_by(AgeDecade) %>% filter(Gender == 'male') %>% summarise(avarage = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
...
```{r}
NHANES %>% select(Gender, AgeDecade, BPSysAve)%>% group_by(AgeDecade, Gender) %>% summarise(avarage = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
...
```{r}
NHANES%>% filter(AgeDecade == " 40-49") %>% group_by(Race1) %>% summarize(average= mean(BPSysAve, na.rm = TRUE)) %>% arrange(average)
```
...
case_when(x < 0 ~ "Negative", 
          x > 0 ~ "Positive", 
          TRUE  ~ "Zero")
ifelse(x<0,"Negative", ifelse(x > 0,"Positive", "Zero" ) )
```
...
```{r}
murders_tribble <- as_tibble(murders)
```
...
```{r}
d%>% group_by(ASD, Effect)%>% select(ASD,Effect)%>% filter(ASD %in% c("Y", "N"))%>% summarise(ASD , Effect)  %>% count()
```
